...
boolean luminosity::add_sensor(uint8_t adr)
{
   ...
   */
	if (TSL2561::is_device(adr) == true)
	{
		...
		{
			log_debug(F("\n[luminosity] ###ERROR at TSL2561 startup ... removing instance ..."));
			log_flush();
			free(cur_sensor);
			cur_sensor = NULL;
		}
	...
	else if (MAX44009::is_device(adr) == true)
	{
		...
		if (cur_sensor->begin(adr) != true)
		{
			log_debug(F("\n[luminosity] ###ERROR at MAX44009 startup ... removing instance ..."));
			log_flush();
			free(cur_sensor);
			cur_sensor = NULL;
		}
...
{
	log_info(F("\n[luminosity] received COMMAND to process ..."));
	log_flush();
	...
	for (JsonObject::iterator it = root.begin(); it != root.end(); ++it)
	{
...

		{
			...
			if (strncmp_P(it->key().c_str(), _key, strlen_P(_key)) == 0)
			{
				...
				if (_key_order)
					_processOrder(_key_order, &_key_value);
				continue;
			}
		}

		// unknown key ?!?!
		log_error(F("\n[luminosity][callback] unknown key: "));
		log_debug((const char *)(it->key().c_str()));
		log_flush();
	}
}
...
boolean luminosity::loadSensoConfig(senso *sp)
{
	...
	log_debug(F("\n[luminosity] loading sensOCampus config is NOT YET IMPLEMENTED!"));
	log_flush();
	return false;
	...
	log_debug(F("\n[luminosity] (re)loading config file (if any)"));
	log_flush();
	loadConfig();
   ...
   */
	...
	for (uint8_t cur_sensor = 0; cur_sensor < _sensors_count; cur_sensor++)
	{
		...
		if (!_sensor[cur_sensor]->acquire(&value))
		{
			...
			log_warning(_sensor[cur_sensor]->subID());
			log_flush();
			continue;
		}
    ...
    */
		...
		else
		{
			...
			log_error(F("\n[luminosity] ERROR failure MQTT msg delivery :("));
			log_flush();
			return false;
		}
...
bool luminosity::_processOrder(const char *order, int *value)
{
	...
	log_error(F("\n[luminosity][callback] unknown order: "));
	log_debug(order);
	log_flush();
	return false;
}
...
bool luminosity::loadConfig(void)
{
	...
	if (!SPIFFS.exists(MODULE_CONFIG_FILE(MQTT_MODULE_NAME)))
		return false;
	...
	log_info(F("\n[luminosity] load JSON config file"));
	log_flush();
